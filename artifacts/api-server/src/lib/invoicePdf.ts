...
export function generateInvoicePdf(data: InvoiceData, brandCfg?: InvoiceBrandConfig): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    if (logoBuf) {
      try {
        doc.roundedRect(L - 6, 13, 216, 54, 8).fill(WHITE);
        doc.image(logoBuf, L, 21, { height: 40 });
      } catch {
    ...
    doc.text("TOTAL DUE", tX, lblY, { width: lW, lineBreak: false });
    doc.font("Helvetica-Bold").fontSize(13).fillColor(WHITE);
    doc.text(fmt(credit > 0 ? totalAfterCred : Number(data.total)),
      tX + lW, amtY, { width: vW, align: "right", lineBreak: false });
    y += PILL_H + 8;
    ...
    doc.text(brandCfg?.ceoName || "Muhammad Arslan", sigX + 8, y + 16, { lineBreak: false });

    doc.font("Helvetica").fontSize(8).fillColor(DARK);
    doc.text(`Founder & CEO, ${siteName}`, sigX + 8, y + 31, { lineBreak: false });
