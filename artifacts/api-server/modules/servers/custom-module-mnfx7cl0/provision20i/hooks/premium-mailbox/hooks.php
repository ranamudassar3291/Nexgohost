...
const PREMIUM_MAILBOX_PRODUCT_NAME = "Premium Mailbox";
...
add_hook("ClientAreaPageCart", 1, function(array $vars) {
    ...
    if ($product->name !== PREMIUM_MAILBOX_PRODUCT_NAME) {
        return [];
    }
...
(function(){
    ...
    select.addEventListener("change", function(e) {
        ...
        // Fetch mailboxes
        fetch(`index.php?action=fetchmailboxes&domain=\${e.target.value}&p=pm`)
            .then(r => r.json())
            .then(data => {
                notice.remove();
                if (
