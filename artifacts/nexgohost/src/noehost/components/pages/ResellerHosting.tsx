...
const ResellerHosting: React.FC = () => {
  ...
  const plans = plansLoading ? [] : (apiPlans.length > 0
    ? apiPlans.map((p, i) => ({
        ...
        btnUrl: '',
        specs: [
          ...(p.diskSpace ? [`${p.diskSpace} NVMe SSD Storage`] : []),
          ...(p.bandwidth ? [`${p.bandwidth} Monthly Bandwidth`] : []),
          'Free SSL for All Clients',
          'WHM Control Panel Included',
          'cPanel for Each Client',
              ...
              <div className="flex items-center justify-center">
                <div className="inline-flex items-center bg-white/8 border border-white/10 rounded-xl p-1 gap-1">
                  ...
                  <button onClick={() => setYearly(true)} className={`px-5 py-2 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-2 ${yearly ? 'bg-white text-gray-900 shadow-sm' : 'text-slate-400 hover:text-white'}`}>
                    Annual
                    <span className="bg-emerald-500 text-white text-[10px] font-black px-1.5 py-0.5 rounded-full leading-none">Save 30%</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              ...
              <div>
                <div className="flex justify-between mb-3">
                  ...
                  <span className="text-primary-300 font-black text-sm">{convertFromPKR(avgPrice)}/mo</span>
                </div>
                <input type="range" min={5} max={100} value={avgPrice} onChange={e => setAvgPrice(Number(e.target.value))}
                  className="w-full h-2 bg-white/10 rounded-full appearance-none cursor-pointer accent-primary" />
                <div className="flex justify-between text-xs text-slate-600 mt-1"><span>{convertFromPKR(5)}</span><span>{convertFromPKR(100)}</span></div>
              </div>
            </div>
            <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
              <div className="text-xs font-black text-slate-500 uppercase tracking-widest mb-3">Your Monthly Revenue</div>
              <div className="text-5xl font-black text-white mb-1">{convertFromPKR(clients * avgPrice)}</div>
              <div className="text-xs text-slate-500 font-medium mb-6">from {clients} client{clients !== 1 ? 's' : ''} at {convertFromPKR(avgPrice)}/mo each</div>
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-white/5 rounded-xl p-4 border border-white/5">
                  <div className="text-xs text-slate-500 font-bold mb-1">Your Plan Cost</div>
                  <div className="text-xl font-black text-slate-300">{convertFromPKR(39.99)}<span className="text-xs text-slate-500">/mo</span></div>
                </div>
