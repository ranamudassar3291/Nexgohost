              ...
              {plansLoading && [0,1,2].map(i => (
                <div key={i} className="rounded-2xl border border-slate-200 bg-white overflow-hidden animate-pulse">
                  <div className="p-6 text-center border-b border-slate-100">
                    <div className="h-5 bg-slate-200 rounded w-24 mx-auto mb-3" />
                    <div className="h-3 bg-slate-100 rounded w-40 mx-auto mb-5" />
                    <div className="h-10 bg-slate-200 rounded w-28 mx-auto mb-2" />
                  </div>
                  <div className="p-6 space-y-3">
                    {[0,1,2,3,4].map(j => <div key={j} className="h-3 bg-slate-100 rounded w-full" />)}
                  </div>
                  <div className="px-6 pb-6"><div className="h-11 bg-slate-200 rounded-xl w-full" /></div>
                </div>
              ))}
                  ...
                  <div className="p-6 text-center border-b border-slate-100">
                    ...
                    <div className="mb-1">
                      {yearly && plan.yearlyTotal ? (
                        <>
                          <span className="text-4xl font-black text-slate-900">{convertFromPKR(plan.yearlyTotal)}</span>
                          <span className="text-sm text-slate-400 font-bold ml-1">/yr</span>
                        </>
                      ) : (
                        <span className="text-4xl font-black text-slate-900">{convertFromPKR(plan.monthly)}</span>
                      )}
                    </div>
                    {yearly && plan.yearlyTotal ? (
                      <>
                    ...
                    <button
                      ...
                      className={`block w-full text-center py-2.5 rounded-xl font-black text-sm transition-all ${plan.popular ? 'bg-primary hover:bg-primary-600 text-white shadow-lg shadow-primary/30' : 'bg-transparent hover:bg-slate-50 text-slate-800 border border-slate-300 hover:border-primary/50'}`}
                    >
