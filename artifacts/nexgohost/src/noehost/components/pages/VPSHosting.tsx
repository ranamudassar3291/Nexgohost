...
const VPSHosting: React.FC = () => {
  ...
  const handleOrderNow = (plan: any) => {
    ...
    const planId = raw.id || '';
    if (planId) {
      window.location.href = `/cart/vps?planId=${planId}`;
    } else {
      window.location.href = '/cart/vps';
    }
  };

  const { plans: apiVps, loading: plansLoading } = useVpsPlans();

  const plans = plansLoading ? [] : (apiVps.length > 0
    ? apiVps.map((p, i) => ({
        name: p.name,
        price: p.price,
        yearlyMonthly: p.yearlyPrice ? +(Number(p.yearlyPrice) / 12).toFixed(2) : null,
        yearlyTotal: p.yearlyPrice ? Number(p.yearlyPrice) : null,
        cpu: `${p.cpuCores} vCPU Cores`,
        ram: `${p.ramGb} GB RAM`,
        storage: `${p.storageGb} GB NVMe SSD`,
        bandwidth: p.bandwidthTb ? `${p.bandwidthTb} TB` : 'Unmetered',
        popular: i === Math.min(1, apiVps.length - 1),
        btnText: 'Deploy Now',
        btnUrl: '',
        _raw: p,
      }))
              ...
              {plans.slice(0, 3).map((plan: any, i: number) => (
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ y: -6 }}
                  ...
                  <div className="flex items-baseline gap-1 mb-1">
                    <span className="text-3xl font-black text-white">{convertFromPKR(yearly && plan.yearlyMonthly ? plan.yearlyMonthly : plan.price)}</span>
                    <span className={`text-sm ${plan.popular ? 'text-primary-200' : 'text-slate-500'}`}>/mo</span>
                  </div>
                  {yearly && plan.yearlyTotal && <div className="text-xs text-emerald-400 font-bold mb-4">Billed {convertFromPKR(plan.yearlyTotal)}/yr</div>}
                  {!yearly || !plan.yearlyTotal ? <div className="mb-4" /> : null}
                  ...
                  <button onClick={() => handleOrderNow(plan)} className={`block w-full text-center py-3 rounded-xl font-black text-sm transition-all ${plan.popular ? 'bg-white text-primary hover:bg-primary-50' : 'bg-primary/10 hover:bg-primary/20 text-primary-300 border border-primary/20'}`}>{plan.btnText || 'Deploy Now'}</button>
                </motion.div>
              ))}
            </div>
          </div>
