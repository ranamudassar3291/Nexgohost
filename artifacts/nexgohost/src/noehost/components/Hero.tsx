...
const Hero: React.FC = () => {
  ...
  const { convert, convertFromPKR } = useCurrency();
  ...
  const [tlds, setTlds] = useState<{ name: string; price: string }[]>(
    FEATURED_EXTS.map(ext => ({ name: ext, price: '...' }))
  );
    ...
    fetch('/api/domain-extensions')
      ...
      .then((list: any[]) => {
        ...
        const mapped = FEATURED_EXTS.map(ext => {
          ...
          const pkrPrice = found ? Number(found.registerPrice ?? found.register ?? 0) : 0;
          return {
            name: ext,
            price: pkrPrice > 0 ? convertFromPKR(pkrPrice) : '—',
          };
        });
        setTlds(mapped);
      })
