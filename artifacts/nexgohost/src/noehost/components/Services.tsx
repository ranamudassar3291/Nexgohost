...
const Services: React.FC = () => {
  ...
  return (
    <section id="services" className="py-16 relative overflow-hidden" style={{ background: '#0a0a0d' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {servicesData.items.map((s: any, i: number) => (
            <motion.div
              ...
              whileHover={{ y: -8 }}
              className="p-7 rounded-[24px] border transition-all duration-500 group"
              style={{ background: '#0e0e14', borderColor: 'rgba(103,61,230,0.15)' }}
              onMouseEnter={e => (e.currentTarget.style.borderColor = 'rgba(103,61,230,0.5)')}
              onMouseLeave={e => (e.currentTarget.style.borderColor = 'rgba(103,61,230,0.15)')}
            >
              <div className={`w-12 h-12 ${iconBgsDark[i % iconBgsDark.length]} rounded-xl flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-500`}>
