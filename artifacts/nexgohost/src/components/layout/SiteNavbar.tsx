...
import {
  ...
  Zap, Globe, Cpu, Users, Layout, Info, Mail, ShoppingCart, Home,
} from "lucide-react";
...
const HOSTING_LINKS = [
  { name: "Shared",    label: "Shared Hosting",    href: "/order",           icon: <Server size={18} />, color: "text-blue-500",   key: "shared" },
  { name: "VPS",       label: "VPS Hosting",        href: "/vps-hosting",     icon: <Cpu size={18} />,    color: "text-purple-500", key: "vps" },
  { name: "Reseller",  label: "Reseller Hosting",   href: "/order",           icon: <Users size={18} />,  color: "text-rose-500",   key: "reseller" },
  { name: "WordPress", label: "WordPress Hosting",  href: "/order",           icon: <Layout size={18} />, color: "text-emerald-500",key: "wordpress" },
];

const OTHER_LINKS = [
  { name: "Home",    href: "/",                      icon: <Home size={18} />,   color: "text-primary" },
  { name: "Domains", href: "/client/domain-search",  icon: <Globe size={18} />,  color: "text-cyan-500" },
  { name: "About",   href: "/about-us",               icon: <Info size={18} />,   color: "text-sky-500" },
  { name: "Contact", href: "/contact",                icon: <Mail size={18} />,   color: "text-teal-500" },
];
...
export function SiteNavbar() {
  ...
  return (
    <nav className={`transition-all duration-300 w-full z-[100] ${
      isScrolled ? "bg-white py-3 shadow-xl" : "bg-[#0F172A]/80 backdrop-blur-sm py-5"
    }`}>
      <div className="container mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-12">
          ...
          <div className="hidden lg:flex items-center gap-6">
            {/* Home */}
            <Link href="/" className={`flex items-center gap-2 font-black text-xs transition-all py-2 uppercase tracking-widest group ${
              isScrolled ? "text-slate-600 hover:text-primary" : "text-slate-200 hover:text-white"
            }`}>
              <span className="text-primary group-hover:scale-110 transition-transform"><Zap size={18} /></span>
              Home
            </Link>
            ...
            <div ref={hostingRef} className="relative">
              ...
              >
                <span className="text-primary group-hover:scale-110 transition-transform"><Server size={18} /></span>
                Hosting
              ...
              {hostingOpen && (
                <div className="absolute top-full left-0 mt-3 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden" style={{ minWidth: "340px" }}>
                        ...
                        <Link key={link.key} href={link.href} onClick={() => setHostingOpen(false)}
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              <span className="text-sm font-bold text-slate-800 group-hover:text-primary transition-colors">{link.label}</span>
                              {meta.badge && (
                  ...
                  <div className="border-t border-slate-100 px-5 py-3.5 bg-slate-50/60 flex items-center justify-between">
                    ...
                    <Link href="/#pricing" onClick={() => setHostingOpen(false)} className="text-xs font-black text-primary hover:underline">Compare all plans →</Link>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* Other links */}
            {OTHER_LINKS.filter(l => l.name !== "Home").map(link => (
        ...
        <div className="hidden lg:flex items-center gap-6">
          ...
          <button onClick={() => setLocation(user ? "/client/cart" : "/client/login")}
            className={`relative p-2.5 rounded-xl border transition-all ${
              isScrolled
                ? "bg-slate-50 border-slate-100 text-slate-500 hover:text-primary hover:bg-primary/5"
      ...
      {/* Mobile Menu — always white background per original design */}
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-t border-slate-100 shadow-2xl p-8 flex flex-col gap-6 z-[100] max-h-[85vh] overflow-y-auto animate-in slide-in-from-top-4 duration-300">
          {/* Home */}
          <Link href="/" onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-4 group">
            <div className="p-3 rounded-xl bg-slate-50 text-primary group-hover:bg-primary group-hover:text-white transition-all"><Zap size={18} /></div>
            <span className="text-sm font-black text-slate-800 group-hover:text-primary transition-colors uppercase tracking-widest">Home</span>
          </Link>

          {/* Hosting */}
          <div>
            <button onClick={() => setMobileHostingOpen(o => !o)} className="flex items-center gap-4 group w-full">
              <div className="p-3 rounded-xl bg-slate-50 text-primary group-hover:bg-primary group-hover:text-white transition-all"><Server size={18} /></div>
              <span className="text-sm font-black text-slate-800 uppercase tracking-widest flex-1 text-left">Hosting</span>
              ...
              <div className="mt-3 flex flex-col gap-1">
                {HOSTING_LINKS.map(link => (
                  <Link key={link.key} href={link.href} onClick={() => setMobileMenuOpen(false)}
                    ...
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-slate-100 group-hover:bg-primary/10 transition-all ${link.color}`}>{link.icon}</div>
                    <div>
                      <span className="text-sm font-bold text-slate-800 group-hover:text-primary transition-colors">{link.label}</span>
                      <p className="text-xs text-slate-400 font-medium">{HOSTING_META[link.key]?.desc}</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {/* Other links */}
          {OTHER_LINKS.filter(l => l.name !== "Home").map(link => (
            <Link key={link.name} href={link.href} onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-4 group">
              <div className={`p-3 rounded-xl bg-slate-50 ${link.color} group-hover:bg-primary group-hover:text-white transition-all`}>{link.icon}</div>
