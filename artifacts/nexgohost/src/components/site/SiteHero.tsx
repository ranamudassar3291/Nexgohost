...
import { CheckCircle2, Star, Zap, Server, Shield, Database, Wifi, Activity, Lock } from "lucide-react";
import { useCurrency } from "@/context/CurrencyProvider";
import { SiteDomainChecker } from "./SiteDomainChecker";
        ...
        {[
          ...
          <div key={i} className="flex items-center gap-3 bg-white/5 rounded-xl px-4 py-2.5 border border-white/5">
            ...
            <div className="w-20 h-1.5 bg-white/10 rounded-full overflow-hidden">
              <div className={`h-full ${s.bar} rounded-full`} style={{ background: "linear-gradient(to right, #6A62FE, #8A7FFE)" }} />
            </div>
        ...
        <div className="bg-white/5 rounded-2xl p-4 border border-white/5 flex flex-col gap-2">
          ...
          {[70, 45, 85, 55].map((h, i) => (
            <div key={i} className="flex items-end gap-0.5">
              {[...Array(12)].map((_, j) => (
                <div key={j} className="flex-1 bg-primary/30 rounded-sm" style={{ height: `${Math.random() * h + 8}px` }} />
              ))}
            </div>
          ))}
        </div>
  ...
  return (
    <section className="bg-black pt-28 pb-16 overflow-hidden relative min-h-screen flex flex-col items-center">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-20 left-20 w-40 h-px bg-gradient-to-r from-primary to-transparent" />
        <div className="absolute top-20 left-20 w-px h-40 bg-gradient-to-b from-primary to-transparent" />
        <div className="absolute top-[19px] left-[19px] w-2 h-2 bg-primary rounded-full shadow-[0_0_15px_rgba(106,98,254,0.8)]" />
        <div className="absolute bottom-20 right-20 w-40 h-px bg-gradient-to-l from-primary to-transparent" />
        <div className="absolute bottom-20 right-20 w-px h-40 bg-gradient-to-t from-primary to-transparent" />
        <div className="absolute bottom-[19px] right-[19px] w-2 h-2 bg-primary rounded-full shadow-[0_0_15px_rgba(106,98,254,0.8)]" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-10 mb-14">
          ...
          <div className="lg:w-1/2 text-center lg:text-left">
            ...
            >
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 text-primary text-xs font-black mb-8 border border-primary/20 backdrop-blur-md uppercase tracking-widest">
                <Star size={14} className="fill-primary" />
                Special Offer: Save 75% Today
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-8 mb-16">
                <div className="flex flex-col items-center lg:items-start">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-white text-4xl font-black">{convert(1.99)}</span>
                    <span className="text-slate-500 text-sm font-bold">/mo</span>
                  </div>
                </div>
                <Link href="/order"
                  className="w-full sm:w-auto px-12 py-5 bg-primary hover:bg-primary-600 text-white rounded-xl font-black transition-all shadow-2xl shadow-primary/40 text-lg group flex items-center justify-center gap-3 text-center">
                  Get Started
              ...
              <div className="pt-8 border-t border-white/5 flex flex-col sm:flex-row items-center gap-8 justify-center lg:justify-start">
                <div className="flex -space-x-3">
                  {[
                    ...
                    <img key={i} src={url} alt="User" className="w-10 h-10 rounded-full border-2 border-black object-cover" />
                  ))}
                  <div className="w-10 h-10 rounded-full border-2 border-black bg-[#0F172A] flex items-center justify-center text-[10px] font-black text-white">
                    +2M
                  </div>
                </div>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                {FEATURED_TLDS.map(tld => (
                  <div key={tld.name} className="flex items-center gap-3 px-5 py-3 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-all">
                    ...
                    <span className="text-[#FFD700] font-black">{convert(tld.price)}</span>
                  </div>
                ))}
              </div>
            </div>

            <SiteDomainChecker variant="hero" placeholder="Search for your dream domain..." />
          </div>
        </motion.div>
      </div>
    </section>
