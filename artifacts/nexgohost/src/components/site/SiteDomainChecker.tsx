...
const QUICK_TLDS = [".com", ".net", ".org", ".pk", ".store", ".io"];

export function SiteDomainChecker({ variant = "page", placeholder = "Search for your dream domain..." }: Props) {
  ...
  const handleSearch = async (e: React.FormEvent, overrideQuery?: string) => {
    ...
    setSearched(q);
    try {
      const sld = q.replace(/\.[^.]+$/, "").replace(/^\./, "");
      const res = await fetch(`/api/domains/availability?domain=${encodeURIComponent(sld)}`);
      if (!res.ok) throw new Error("API error");
      const data: { name: string; results: { tld: string; available: boolean; registrationPrice?: number; transferPrice?: number }[] } = await res.json();
      const mapped: DomainResult[] = (data.results || []).map(r => ({
        domain: `${sld}${r.tld}`,
        sld,
        tld: r.tld,
        available: r.available,
        registerPrice: r.registrationPrice ?? 0,
        transferPrice: r.transferPrice ?? 0,
      }));
      setResults(mapped);
    } catch {
      setResults([]);
        ...
        <div className={`flex flex-wrap gap-2 mt-4 ${variant === "hero" ? "" : "justify-center"}`}>
          {QUICK_TLDS.map(tld => (
            <button
              key={tld}
              type="button"
              onClick={() => setQuery(prev => prev.replace(/\.[a-z]+$/, "") + tld)}
        ...
        {results.length > 0 && (
          ...
          >
            <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => { setResults([]); setSearched(""); }} className="text-slate-400 hover:text-slate-600 transition-colors">
                <XCircle size={20} />
              </button>
            </div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
                    <div className="text-right hidden sm:block">
                      <div className={`text-sm sm:text-base font-black ${res.available ? "text-primary" : "text-orange-500"}`}>
                        {formatPrice(res.available ? res.registerPrice : res.transferPrice)}
                      </div>
                    ...
                    {res.available ? (
                      <a
                        href={`/client/domain-search?domain=${encodeURIComponent(res.domain)}`}
                      ...
                      <a
                        href={`/client/domain-search?transfer=${encodeURIComponent(res.domain)}`}
