...
import {
  ...
  ArrowLeft, Wifi, WifiOff, RefreshCw, Clock, AlertCircle, List, KeyRound,
} from "lucide-react";
...
export default function EmailConfiguration() {
  ...
  const cfg = form ?? settingsData ?? {
    ...
    google_client_id: "", google_configured: false,
  };
        ...
        <div className="xl:col-span-2 space-y-5">
          ...
          {cfg.mailer_type === "smtp" && (
            <form onSubmit={handleSave} className="rounded-2xl border border-border bg-card p-6 space-y-5">
              ...
              {/* Quick-fill SMTP providers */}
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground/80">Quick Setup — Click a provider to auto-fill</label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    <button key={p.name} type="button"
                      ...
                      className="flex flex-col gap-0.5 p-3 rounded-xl border border-border hover:border-primary/50 hover:bg-primary/5 transition-all text-left">
