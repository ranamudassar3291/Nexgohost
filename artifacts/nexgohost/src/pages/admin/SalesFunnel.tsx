...
const EMPTY_SALE: Partial<FlashSale> = {
  ...
  ends_at: "", bg_color: "#0F172A", accent_color: "#6366F1", is_active: true,
};
  ...
  return (
    <div style={{ padding: "32px 28px", minHeight: "100vh", background: "#F8FAFC" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        ...
        {tab === "flash" && (
          <button onClick={openCreate} style={{ display: "flex", alignItems: "center", gap: 8, background: "#4F46E5", color: "#fff", border: "none", borderRadius: 10, padding: "10px 18px", fontWeight: 600, cursor: "pointer", fontSize: 14 }}>
        ...
        <div>
          {flashLoading ? (
            ...
            <div style={{ textAlign: "center", padding: 60, background: "#fff", borderRadius: 16, border: "1px dashed #E5E7EB" }}>
              ...
              <button onClick={openCreate} style={{ marginTop: 16, background: "#4F46E5", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", cursor: "pointer", fontWeight: 600 }}>
                Create Flash Sale
              </button>
            </div>
          ) : (
              ...
              {flashSales.map(sale => {
                ...
                return (
                  <div key={sale.id} style={{ background: "#fff", borderRadius: 16, padding: "20px 24px", border: "1px solid #E5E7EB", display: "flex", alignItems: "center", gap: 20 }}>
                    {/* Colour swatch */}
                    <div style={{ width: 48, height: 48, borderRadius: 12, background: sale.bg_color || "#0F172A", flexShrink: 0, border: `3px solid ${sale.accent_color || "#6366F1"}` }} />
                    {/* Info */}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                        ...
                        <span style={{ background: "#EEF2FF", color: "#4F46E5", fontSize: 11, padding: "2px 8px", borderRadius: 20, fontWeight: 600 }}>/{sale.slug}</span>
                      </div>
                    ...
                    <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                      ...
                      <button onClick={() => openEdit(sale)}
                        style={{ padding: "7px 14px", borderRadius: 8, border: "1px solid #E5E7EB", background: "#F9FAFB", cursor: "pointer", color: "#4F46E5", display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 500 }}>
                        <Edit2 size={14} /> Edit
                      </button>
              ...
              <form onSubmit={handleSubmit}
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 24, justifyContent: "flex-end" }}>
                  ...
                  <button type="submit" disabled={saveSale.isPending}
                    style={{ padding: "10px 22px", borderRadius: 10, border: "none", background: "#4F46E5", color: "#fff", cursor: "pointer", fontWeight: 600, opacity: saveSale.isPending ? .7 : 1 }}>
      ...
      {tab === "recovery" && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, marginBottom: 28 }}>
            {[
              { label: "Triggers", value: stats.triggered, icon: <ShoppingCart size={18} />, color: "#4F46E5" },
              ...
              <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
                {["all", "triggered", "converted", "dismissed"].map(s => (
                  <button key={s} onClick={() => setFilterStatus(s)}
                    style={{ padding: "5px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 13, background: filterStatus === s ? "#4F46E5" : "#F3F4F6", color: filterStatus === s ? "#fff" : "#374151" }}>
                    ...
                    {logs.map((log, i) => (
                      <tr key={log.id} style={{ borderTop: "1px solid #F3F4F6", background: i % 2 === 0 ? "#fff" : "#FAFAFA" }}>
                        ...
                        <td style={{ padding: "12px 16px", fontSize: 13 }}>
                          {log.discount_code ? <code style={{ background: "#EEF2FF", color: "#4F46E5", padding: "2px 8px", borderRadius: 6, fontSize: 12 }}>{log.discount_code}</code> : "—"}
                        </td>
