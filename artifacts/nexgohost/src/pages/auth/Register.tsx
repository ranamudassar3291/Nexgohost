  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(o => !o)}
        ...
        <span className="flex-1 text-left truncate">{value.name}{detecting && <span className="ml-2 text-[10px] text-violet-500 animate-pulse">detecting…</span>}</span>
            ...
            <div className="p-2 border-b border-gray-100">
              <div className="relative">
                ...
                <input ref={searchRef} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search country…"
                  className="w-full h-8 pl-7 pr-3 text-xs rounded-lg border border-gray-200 outline-none focus:ring-2 focus:ring-[#6B46C1]/25 focus:border-[#6B46C1] bg-gray-50" />
              </div>
            </div>
            <ul className="max-h-52 overflow-y-auto">
              {filtered.length === 0 ? (
                ...
                <li key={c.code} onClick={() => { onChange(c); setOpen(false); setSearch(""); }}
                  className={`flex items-center gap-3 px-3 py-2 cursor-pointer text-sm transition-colors ${c.code === value.code ? "bg-violet-50 text-violet-700" : "hover:bg-gray-50 text-gray-700"}`}>
...
export default function Register() {
  ...
  function startCountdown(setter: (v: number) => void) {
    setter(60);
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.requiresVerification) {
        ...
        toast({ title: "Verification code sent", description: `Check your inbox at ${form.email}` });
      } else {
        // Skip email verify — go straight to phone verify
        if (form.phone) {
          await sendPhoneOtp(data.token);
        } else {
          login(data.token);
          redirect();
        }
  ...
  async function handleVerifyEmail(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await apiFetch("/api/auth/verify-email", tempToken, { method: "POST", body: JSON.stringify({ code: emailCode }) });
      toast({ title: "Email verified!", description: "Now let's verify your phone number." });
      if (form.phone) {
        await sendPhoneOtp(tempToken);
      } else {
        login(tempToken);
        redirect();
      }
  ...
  async function sendPhoneOtp(token: string) {
    setPhoneSending(true);
    try {
      await apiFetch("/api/auth/send-phone-otp", token, { method: "POST", body: JSON.stringify({ phone: form.phone }) });
      setPhoneSent(true);
      setStep("verify-phone");
      startCountdown(setPhoneCountdown);
      toast({ title: "Code sent!", description: `Check your phone ${form.phone}` });
    } catch (err: any) {
      // Proceed without phone verify if send fails
      login(token);
      redirect();
  ...
  async function handleVerifyPhone(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await apiFetch("/api/auth/verify-phone-otp", tempToken, { method: "POST", body: JSON.stringify({ code: phoneCode }) });
      login(tempToken);
      toast({ title: "Phone verified!", description: "Welcome to Noehost." });
      redirect();
    } catch (err: any) {
      toast({ title: "Verification failed", description: err.message, variant: "destructive" });
    } finally { setLoading(false); }
  ...
  const stepIdx = step === "form" ? 0 : step === "verify-email" ? 1 : 2;
    ...
    <div className="min-h-screen w-full flex items-center justify-center px-4 py-10"
      ...
      <div className="w-full max-w-[520px]">
          ...
          {["Account", "Email", "Phone"].map((label, i) => (
            <div key={i} className="flex items-center gap-2">
              <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold transition-all ${
                i < stepIdx ? "bg-emerald-500 text-white" : i === stepIdx ? "text-white shadow-lg" : "bg-gray-100 text-gray-400"
              }`} style={i === stepIdx ? { background: `linear-gradient(135deg, ${BRAND} 0%, #7C5DE2 100%)` } : {}}>
                {i < stepIdx ? <CheckCircle2 size={14} /> : i + 1}
              </div>
              <span className={`text-xs font-medium hidden sm:block ${i === stepIdx ? "text-violet-700" : i < stepIdx ? "text-emerald-600" : "text-gray-400"}`}>{label}</span>
              {i < 2 && <div className={`w-8 h-px ${i < stepIdx ? "bg-emerald-300" : "bg-gray-200"}`} />}
            </div>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === "form" && (
            <motion.div key="form" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <form onSubmit={handleRegister} className="space-y-4" noValidate>
                ...
                {/* Username — auto from email, editable */}
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5 flex items-center gap-1">
                  ...
                  <div className="relative">
                    ...
                    <input name="username" value={form.username} onChange={set("username")}
                      placeholder="your_username" className={`${inputCls("username")} pl-8`} autoComplete="username" />
                  </div>
                  <p className="mt-1 text-[11px] text-gray-400">You can login with username OR email. Min. 3 characters.</p>
                ...
                <div>
                  ...
                  <PhoneInput value={form.phone} onChange={val => { setForm(prev => ({ ...prev, phone: val })); setFieldErrors(prev => ({ ...prev, phone: undefined })); }}
                ...
                <button type="submit" disabled={loading || (captchaRequired && !captchaToken)}
                  ...
                  {loading ? <><Loader2 size={16} className="animate-spin" /> Creating account…</> : <>Continue <ArrowRight size={15} /></>}
                </button>
              </form>
          ...
          {step === "verify-email" && (
            <motion.div key="verify-email" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
              ...
              <div className="text-center mb-6">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: `${BRAND}18` }}>
                  <Mail size={26} style={{ color: BRAND }} />
                </div>
                <h2 className="text-xl font-bold text-gray-900">Verify your email</h2>
                <p className="text-gray-500 text-sm mt-1">
                  We sent a 6-digit code to <span className="font-semibold text-gray-800">{form.email}</span>
                </p>
                <p className="text-gray-400 text-xs mt-1">Expires in 10 minutes · Check spam if needed</p>
              </div>
              <form onSubmit={handleVerifyEmail} className="space-y-4">
                <OtpInput value={emailCode} onChange={setEmailCode} />
                <button type="submit" disabled={loading || emailCode.length !== 6}
                  className="w-full h-12 rounded-xl text-white text-sm font-semibold transition-all flex items-center justify-center gap-2 disabled:opacity-60 shadow-lg"
                  style={{ background: `linear-gradient(135deg, ${BRAND} 0%, #7C5DE2 100%)` }}>
                  {loading ? <><Loader2 size={16} className="animate-spin" /> Verifying…</> : <><ShieldCheck size={15} /> Verify Email</>}
                </button>
              </form>
              <div className="mt-5 text-center">
                {emailCountdown > 0 ? (
                  <p className="text-sm text-gray-400">Resend in <span className="font-mono font-semibold text-gray-600">{emailCountdown}s</span></p>
                ) : (
                  <button onClick={handleResendEmail} disabled={emailResending}
                    ...
                    {emailResending ? <><RefreshCw size={13} className="animate-spin" /> Sending…</> : "Resend code"}
                  </button>
                )}
              </div>
          ...
          {step === "verify-phone" && (
            <motion.div key="verify-phone" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
              className="bg-white border border-gray-100 rounded-2xl shadow-2xl shadow-violet-500/10 p-8">
              <div className="text-center mb-6">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "#10B98118" }}>
                  <Phone size={26} className="text-emerald-600" />
                </div>
                <h2 className="text-xl font-bold text-gray-900">Verify your phone</h2>
                <p className="text-gray-500 text-sm mt-1">
                  We sent a 6-digit code to <span className="font-semibold text-gray-800">{form.phone}</span>
                </p>
                <p className="text-gray-400 text-xs mt-1">Check your messages or email inbox</p>
              </div>
              <form onSubmit={handleVerifyPhone} className="space-y-4">
                ...
                <button type="submit" disabled={loading || phoneCode.length !== 6}
                  ...
                  {loading ? <><Loader2 size={16} className="animate-spin" /> Verifying…</> : <><ShieldCheck size={15} /> Verify Phone & Finish</>}
                </button>
              </form>
              <div className="mt-5 text-center">
                {phoneCountdown > 0 ? (
                  <p className="text-sm text-gray-400">Resend in <span className="font-mono font-semibold text-gray-600">{phoneCountdown}s</span></p>
                ) : (
                  <button onClick={handleResendPhone} disabled={phoneSending}
                    className="text-sm font-semibold flex items-center gap-1.5 mx-auto hover:underline disabled:opacity-50 text-emerald-600">
                    {phoneSending ? <><RefreshCw size={13} className="animate-spin" /> Sending…</> : "Resend code"}
                  </button>
                )}
              </div>
              <button onClick={() => { login(tempToken); redirect(); }}
                className="mt-4 w-full text-xs text-gray-400 hover:text-gray-600 text-center transition-colors">
                Skip for now →
              </button>
            </motion.div>
          )}
