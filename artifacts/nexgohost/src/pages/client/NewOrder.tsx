...
function tok() { return localStorage.getItem("token") || localStorage.getItem("noehost_token") || ""; }
            ...
            <div key={label} className="flex items-start">
              <div className="flex flex-col items-center w-[76px] sm:w-[90px]">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${
                  done ? "bg-[#6B46C1] text-white shadow-md" :
                  cur  ? "bg-[#6B46C1] text-white ring-4 ring-[#6B46C1]/25 shadow-md" :
                         "bg-gray-100 text-gray-400"
                }`}>
                  {done ? <Check size={15} strokeWidth={2.5}/> : i + 1}
                </div>
                <span className={`mt-1.5 text-[11px] font-semibold text-center leading-tight ${cur ? "text-[#6B46C1]" : done ? "text-gray-600" : "text-gray-400"}`}>
                  {label}
                </span>
              </div>
              {i < displayLabels.length - 1 && (
                <div className={`w-10 sm:w-14 h-0.5 mt-[18px] shrink-0 transition-colors duration-500 ${done ? "bg-[#6B46C1]" : "bg-gray-200"}`}/>
              )}
            </div>
          );
        })}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                    {vpsPlans.map(plan => (
                      <button
                        key={plan.id}
                        onClick={onSelectVps}
                        className="text-left p-4 rounded-xl border border-gray-200 hover:border-[#6B46C1] hover:shadow-md transition-all duration-200 group"
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <p className="text-[14px] font-bold text-gray-900 group-hover:text-[#6B46C1] transition-colors">{plan.name}</p>
            ...
            {displayPlans.map((plan, idx) => {
              ...
              return (
                <button key={plan.id} onClick={() => clickPlan(plan)}
                  ...
                  {/* Header */}
                  <div className={`px-5 pt-7 pb-4 ${isSel ? "border-b border-[#6B46C1]/15" : "border-b border-gray-100"}`}>
            ...
            {[...domResults].filter(r => r.registrationPrice > 0 && (r.showInSuggestions === true || r.tld === domTypedTld)).sort((a, b) => (a.sortOrder ?? 999) - (b.sortOrder ?? 999)).slice(0, 10).map(r => (
              <div key={r.tld}
                className={`flex items-center gap-3 justify-between px-4 py-3.5 bg-white rounded-xl border transition-all ${
                  r.available ? "border-gray-200 hover:border-[#6B46C1]/30" : "border-gray-100 opacity-50"
                }`}>
